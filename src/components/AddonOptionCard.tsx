...
export default function AddonOptionCard({
  ...
  return (
    <button
      ...
      className={`flex w-full flex-col rounded-sm border p-3 text-left transition ${
        selecionado
    ...
    >
      ...
      {servico.shortDescription && (
        <p className="mt-1 line-clamp-2 text-xs text-text-secondary">{servico.shortDescription}</p>
      )}
      <span
